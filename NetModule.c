...
MODULE_LICENSE("GPL");
MODULE_DESCRIPTION("Netfilter Project CS3281");
MODULE_AUTHOR("Wang Christina");
...

{
    ...
    if (procf_buffer[procf_buffer_pos] == 'p') {
        ...
        while ((procf_buffer[i]!='\0') && (procf_buffer[i]!='n') ) {
...
int init_module() {
    ...
    /*
    hook_in/out gets address; either block all or iterates through list
            -if blocked: log using printk and return NF_REJECT
            -else: return NF_ACCEPT
    2 linked lists - incoming to block / outgoing to block
    global flags
            -block all
            -block all incoming
            -block all outgoing
            -int use bitmask for three flags at once
            -or use 2,4,8
    struct file_operations fops = {
            .owner = THIS_MODULE,
            .read = read_proc, // when user wants to read an IP
            .write = write_proc // when user adds an IP
    };

    #1) insmod
    #2) echo "129.24.104.21 outgoing\n" >> /proc/filter // when user adds an IP

    size_t write/read_proc( struct file *p,
                            char __user * buf,
                            size_t count, // size of buffer
                            loff_t *off)
    {
        // need to use copyfromuser in order to read buf because need to validate pointer
        // add the address to your list of filtered addresses
        // list of filtered addresses can be linked list; global so changes to it there are
        // reflected in hook function when it iterates through it
        // also has to be static

    }
    */

    mf_proc_file = proc_create(PROCF_NAME, 0644, NULL);
